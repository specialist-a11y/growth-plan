...
/* Root Variables Default Baseline */
:root {
...
/* Glassmorphism for app containers over background photos */
.card, .hero, .panel, .modal-card, .tab-btn, .table-card, .reward-card, .fixed-routine-card {
  background: var(--theme-surface, rgba(30, 41, 59, 0.78)) !important;
...
NEW_LOAD_CONFIG = """function loadConfig() {
  try {
    const cfg = JSON.parse(localStorage.getItem(configKey()));
    if (cfg) {
      if (cfg.rewardsCatalog) rewardsCatalog = cfg.rewardsCatalog;
      if (cfg.redemptionLog) redemptionLog = cfg.redemptionLog;
      if (cfg.selectedAvatar) selectedAvatar = cfg.selectedAvatar;
      ...
      if (cfg.parentAccount) parentAccount = cfg.parentAccount;
    }
  } catch(e){}
...
def process_file(filepath):
    ...
    # 4. Remove old openThemeModal and setThemeScheme functions in existing script
    content = re.sub(r'function openThemeModal\s*\(\)\s*\{.*?\n\}', '', content, flags=re.DOTALL)
    content = re.sub(r'function setThemeScheme\s*\(.*?\)\s*\{.*?\n\}', '', content, flags=re.DOTALL)

    # 5. Replace loadConfig & saveConfig
    content = re.sub(r'function loadConfig\(\)\s*\{.*?function saveConfig\(\)\s*\{.*?\n\}', NEW_LOAD_CONFIG, content, flags=re.DOTALL)

    # 6. Inject THEME_JS inside script right before loadConfig() definition
    if '// --- PHOTO THEME CUSTOMIZATION SYSTEM ---' not in content:
        content = content.replace('function loadConfig() {', f'{THEME_JS}\n\nfunction loadConfig() {{')
