...
# CSS snippet to inject into <head> or main <style>
THEME_CSS = """
/* Photo Theme Customization System Styles */
#appBgPhotoLayer {
  position: fixed;
  top: 0;
  left: 0;
...
/* Dynamic theme cascading variables */
:root {
  --theme-primary: #FF2D2D;
  --theme-secondary: #2D9CFF;
  --theme-accent: #C8CDD3;
  --theme-bg: #090909;
  --theme-surface: rgba(18, 18, 22, 0.82);
  --theme-text: #FFFFFF;
...
<div id="themeModal" class="modal">
  <div class="modal-card theme-customizer-modal" style="max-width: 980px; width: 95vw; max-height: 90vh; display: flex; flex-direction: column; padding: 0; overflow: hidden; border-radius: 24px; border: 1px solid var(--theme-border, rgba(255,255,255,0.2)); background: rgba(15, 23, 42, 0.94); backdrop-filter: blur(24px); box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.7);">
    ...
    <div class="theme-modal-header" style="padding: 20px 24px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: space-between; background: rgba(0,0,0,0.3);">
      <div>
        <h2 style="margin: 0; font-size: 22px; font-weight: 900; letter-spacing: -0.5px; background: linear-gradient(135deg, #ffffff 0%, #cbd5e1 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
          ✨ Choose Your Vibe
        </h2>
...
# JavaScript object & logic snippet
THEME_JS = f"""
...
let themeIntensity = 'balanced'; // soft, balanced, vibrant
...
function applyAndSaveTheme() {{
  ...
  if (typeof showToast === 'function') {{
    showToast(`✨ Applied Theme: ${{THEMES_CONFIG[savedTheme].name}}`);
  }}
}}
...
def process_file(filepath):
    ...
    # 1. Inject background layers after <body>
    ...
    # 3. Replace old themeModal HTML
    modal_match = re.search(r'<div id="themeModal".*?</div>\s*</div>\s*</div>', content, re.DOTALL)
    if modal_match:
        content = content.replace(modal_match.group(0), THEME_MODAL_HTML)

    # 4. Replace old openThemeModal and setThemeScheme functions in script
    # Look for existing openThemeModal definition
    open_modal_match = re.search(r'function openThemeModal\s*\(\)\s*\{.*?\n\}', content, re.DOTALL)
    if open_modal_match:
        content = content.replace(open_modal_match.group(0), '')

    set_scheme_match = re.search(r'function setThemeScheme\s*\(.*?\)\s*\{.*?\n\}', content, re.DOTALL)
    if set_scheme_match:
        content = content.replace(set_scheme_match.group(0), '')

    # Inject THEME_JS before </body> or inside the main script tag
    if '// --- PHOTO THEME CUSTOMIZATION SYSTEM ---' not in content:
        content = content.replace('</script>\n</body>', f'\n{THEME_JS}\n</script>\n</body>')
        if '// --- PHOTO THEME CUSTOMIZATION SYSTEM ---' not in content:
            content = content.replace('</body>', f'<script>\n{THEME_JS}\n</script>\n</body>')

    # Update saveConfig and loadConfig in the file to handle theme object
    content = content.replace('selectedTheme', 'savedTheme, themeIntensity, themeMotion, themeNoBg')
